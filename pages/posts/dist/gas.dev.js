...
/* slackと連携しspreadsheetに勤怠を書き込むプログラム
...
loadGSTemplate = function loadGSTemplate() {
  var GSTemplate = function GSTemplate(spreadsheet) {
    this.spreadsheet = spreadsheet; // メッセージテンプレート設定
    ...
    if (!this.sheet) {
      ...
      if (!this.sheet) {
        ...
        this.sheet.getRange("A1:L2").setValues([["出勤", "出勤更新", "退勤", "退勤更新", "休暇", "休暇更新", "休暇おわり", "休暇おわり更新", "出勤中", "出勤なし", "休暇中", "休暇なし", "出勤確認", "退勤確認"], ["<@#1> おはようございます (#2)", "<@#1> 出勤時間を#2へ変更しました", "<@#1> お疲れ様でした (#2)", "<@#1> 退勤時間を#2へ変更しました", "<@#1> #2を休暇として登録しました", "<@#1> #2の休暇を取り消しました", "#1が出勤しています", "全員退勤しています", "#1は#2が休暇です", "#1に休暇の人はいません", "今日は休暇ですか？ #1", "退勤しましたか？ #1"]]);
      }
    }
  }; // テンプレートからメッセージを生成
...
loadGSTimesheets = function loadGSTimesheets() {
    ...
    if (!sheet) {
      ...
      if (!sheet) {
        ...
        if (sheet.getLastRow() == 0) {
          ...
          sheet.getRange("A1:C" + properties.length).setValues(properties); // ヘッダの書き出し
  ...
  GSTimesheets.prototype._getRowNo = function (username, date) {
    ...
    var startAt = DateUtils.parseDate(this.scheme.properties.get("開始日"));
  ...
  GSTimesheets.prototype.set = function (username, date, params) {
    ...
    _.extend(row, _.pick(params, 'signIn', 'signOut', 'rest', 'restOff', 'takeBreak', 'restart'));

    var sheet = this._getSheet(username);

    var rowNo = this._getRowNo(username, date);

    var data = [DateUtils.toDate(date), row.signIn, row.signOut, row.rest, row.restOff, row.takeBreak, row.restart].map(function (v) {
      return v == null ? '' : v;
    });
...
function setUp() {
  initLibraries(); // spreadsheetが無かったら初期化
  ...
  if (!global_settings.get('spreadsheet')) {
    ...
    settings.setNote('無視するユーザ', '反応をしないユーザを,区切りで設定する。botは必ず指定してください。'); // apiKey が設定されている場合のみ自動追加
    ...
    settings.setNote('休日', '日付を,区切りで。来年までは自動設定されているので、以後は適当に更新してください'); // メッセージ用のシートを作成

    new GSTemplate(spreadsheet); // 毎日11時頃に出勤してるかチェックする

    ScriptApp.newTrigger('confirmSignIn').timeBased().everyDays(1).atHour(11).create(); // 毎日22時頃に退勤してるかチェックする

    ScriptApp.newTrigger('confirmSignOut').timeBased().everyDays(1).atHour(22).create();
  }
}
...
loadSlack = function loadSlack() {
  ...
  _.extend(Slack.prototype, EventListener.prototype); // 受信したメッセージをtimesheetsに投げる


  Slack.prototype.receiveMessage = function (message) {
    var username = String(message.user_name);
    var body = String(message['text']); // 特定のアカウントには反応しない

    var ignore_users = (this.settings.get("無視するユーザ") || '').toLowerCase().replace(/^\s*(.*?)\s*$/, "$1").split(/\s*,\s*/);
    if (_.contains(ignore_users, username.toLowerCase())) return; // -で始まるメッセージも無視
...
loadTimesheets = function loadTimesheets(exports) {
  ...
  Timesheets.prototype.receiveMessage = function (username, message) {
    ...
    var commands = [['actionSignOut', /(bye)/], ['actionWhoIsOff', /(だれ|誰|who\s*is).*(やす(ま|み|む))/], ['actionWhoIsIn', /(だれ|誰|who\s*is)/], ['actionRest', /(break)/], ['actionRestOff', /(restart)/], ['actionTakeBreak', /(休憩|休)/], ['actionRestart', /(再開)/], ['actionSignIn', /(hi)/], ['confirmSignIn', /__confirmSignIn__/], ['confirmSignOut', /__confirmSignOut__/]]; // メッセージを元にメソッドを探す
    ...
    if (this.datetime) {
      ...
      if (!data.rest || data.rest === '-') {
        ...
        if (!!this.time) {
          ...
          this.responder.template("休憩更新", username, this.datetimeStr);
        }
      }
    }
  }; // 休憩おわり申請
  ...
  Timesheets.prototype.actionRestOff = function (username, message) {
    if (this.datetime) {
      var data = this.storage.get(username, this.datetime);

      if (!data.restOff || data.restOff === '-') {
        this.storage.set(username, this.datetime, {
          restOff: this.datetime
        });
        this.responder.template("休憩おわり", username, this.datetimeStr);
      } else {
        // 更新の場合は時間を明示する必要がある
        if (!!this.time) {
          this.storage.set(username, this.datetime, {
          ...
          this.responder.template("休憩おわり更新", username, this.datetimeStr);
        }
      }
    }
  }; // 休憩2回目申請
  ...
  Timesheets.prototype.actionTakeBreak = function (username, message) {
    if (this.datetime) {
      var data = this.storage.get(username, this.datetime);

      if (!data.takeBreak || data.takeBreak === '-') {
        this.storage.set(username, this.datetime, {
          takeBreak: this.datetime
        });
        this.responder.template("休憩2", username, this.datetimeStr);
      } else {
        // 更新の場合は時間を明示する必要がある
        if (!!this.time) {
          this.storage.set(username, this.datetime, {
          ...
          this.responder.template("休憩2更新", username, this.datetimeStr);
        }
      }
    }
  }; // 休憩２回目おわり申請
  ...
  Timesheets.prototype.actionRestart = function (username, message) {
    if (this.datetime) {
      var data = this.storage.get(username, this.datetime);

      if (!data.restart || data.restart === '-') {
        this.storage.set(username, this.datetime, {
          restart: this.datetime
        });
        this.responder.template("休憩2おわり", username, this.datetimeStr);
      } else {
        // 更新の場合は時間を明示する必要がある
        if (!!this.time) {
          this.storage.set(username, this.datetime, {
          ...
          this.responder.template("休憩2おわり更新", username, this.datetimeStr);
        }
      }
    }
  }; // 出勤中
...
(function () {
  ...
  h.bind = function (n, t) {
    ...
    return r = a.call(arguments, 2), _e = function e() {
      if (!(this instanceof _e)) return n.apply(t, r.concat(a.call(arguments)));
