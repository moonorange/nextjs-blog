...
loadDateUtils = function () {
      ...
      if(!this.sheet) {
        ...
        if(!this.sheet) {
          ...
          this.sheet.getRange("A1:L2").setValues([
            [
              "出勤", "出勤更新", "退勤", "退勤更新", "休暇", "休暇更新", "休暇おわり", "休暇おわり更新",
  ...
  loadGSTimesheets = function () {
    ...
    GSTimesheets.prototype.get = function(username, date) {
      ...
      return({ user: username, date: row[0], signIn: row[1], signOut: row[2], rest: row[3], restOff: row[4], takeBreak: row[5], restart: row[6] });
    };
  ...
  */
  ...
  loadTimesheets = function (exports) {
    ...
    Timesheets.prototype.receiveMessage = function(username, message) {
      ...
      var commands = [
        ['actionSignOut', /(bye)/],
        ['actionWhoIsOff', /(だれ|誰|who\s*is).*(やす(ま|み|む))/],
        ['actionWhoIsIn', /(だれ|誰|who\s*is)/],
        ['actionRest', /(break)/],
        ['actionRestOff', /(restart)/],
        ['actionTakeBreak', /(休憩|休)/],
        ['actionRestart', /(再開)/],
        ['actionSignIn', /(hi)/],
    ...
    Timesheets.prototype.actionRest = function(username, message) {
      if(this.datetime) {
        ...
        if(!data.rest || data.rest === '-') {
          this.storage.set(username, this.datetime, {rest: this.datetime});
    ...
    Timesheets.prototype.actionRestOff = function(username, message) {
      if(this.datetime) {
        ...
        if(!data.restOff || data.restOff === '-') {
          this.storage.set(username, this.datetime, {restOff: this.datetime});
    ...
    Timesheets.prototype.actionTakeBreak = function(username, message) {
      if(this.datetime) {
        ...
        if(!data.takeBreak || data.takeBreak === '-') {
          this.storage.set(username, this.datetime, {takeBreak: this.datetime});
    ...
    Timesheets.prototype.actionRestart = function(username, message) {
      if(this.datetime) {
        ...
        if(!data.restart || data.restart === '-') {
          this.storage.set(username, this.datetime, {restart: this.datetime});
